...
void solve() {
    ...
    while (q--) {
        int k;
        cin >> k;
        // Adapted from the usual count unique substrings problem
        int64_t cnt_unique = 0;
        for (int i = 1; i < n; i++) {
            // exclude last char (do not count its substrings)
            int to_add = (n - sa.SA[i] - 1) - sa.LCP[i];
            if (cnt_unique + to_add >= k) {
                cout << s.substr(sa.SA[i], sa.LCP[i] + k - cnt_unique) << '\n';
                break;
            }
            cnt_unique += to_add;
        }
