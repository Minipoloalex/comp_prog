...
void solve() {
    ...
    vector<int> roots = {0};
    for (int i = 0; i < n; i++) {
        roots.push_back(st.update(roots.back(), a[i].second, +1));
    }
    auto possible = [&](int med, int l, int r, int k) {
        // does it have >= med values in [l, r]
        return st.query(roots[med], l, r) >= k;
    };
    while (m--) {
        int l, r, k;
        cin >> l >> r >> k;
