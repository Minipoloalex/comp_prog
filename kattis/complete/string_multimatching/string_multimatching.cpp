...
void solve() {
    // is it necessary for this problem to have string matching in O(m + logN) instead of O(m * logN)?
    // NO, since patterns at most size 1e5 (with NrP being number of patterns and m the size of patterns)
    // Patterns can be small and many or big and few

    // So, it's (m + logN) * NrP = m * NrP + logN * NrP    
    // Other string matching is: m * logN * NrP = (m * NrP) * logN
    // m * NrP can only be 1e5

    s += '\1';
    SuffixArray sa(s.c_str(), (int) s.size());
    for (auto &p: ps) {
        auto [lb, ub] = sa.stringMatching(p.c_str());
        if (lb == -1 && ub == -1) {
            cout << '\n';
            continue;
        }
        vector<int> ids(ub - lb + 1);
        for (int i = lb; i <= ub; i++) {
            ids[i - lb] = sa.SA[i];
        }
