...
pair<double, int> get_min_angle(const vector<point> &poly) {
    double min_angle = 500;
    int idx_min_ang = -1;
    for (int i = 0; i < (int) poly.size(); i++) {
        double ang = angle(poly[i], poly[(i + 1) % poly.size()], poly[(i + 2) % poly.size()]);
        if (ang < min_angle) {
            idx_min_ang = (i + 1) % poly.size();
            min_angle = ang;
        }
    }
    return {min_angle, idx_min_ang};
}
...
int main() {
    ...
    while (cin >> n) {
        ...
        vector<point> poly(n);
        for (int i = 0; i < n; i++) {
            cin >> poly[i].x >> poly[i].y;  // [0, 15000]
            // cw order, no coincident points
        }
        ...
        // }
        // cout << endl;
        while (poly.size() > 3) {   // O(n^2)
            auto [min_angle, idx_min_ang] = get_min_angle(poly);

            vector<point> new_poly = poly;
            // cout << new_poly[idx_min_ang - 1] << endl;
            new_poly.erase(find(new_poly.begin(), new_poly.end(), new_poly[idx_min_ang]));
            auto [new_min_angle, idx_new_max_ang] = get_min_angle(new_poly);
            
            if (new_min_angle < min_angle) break;   // if sharper stop and return previous polygon
            poly = new_poly;
        }
        reverse(poly.begin(), poly.end());
