...
vector<point> remove_duplicates(const vector<point> &Pts) {
    if (Pts.empty()) return {};
    vector<point> res{Pts[0]};    
    for (int i = 1; i < (int) Pts.size(); i++) {
        if (!(Pts[i] == Pts[i - 1])) res.push_back(Pts[i]);
    }
    return res;
}
...
int main() {
    ...
    while (cin >> n) {  // [1, 10^5]
        ...
        vector<point> pts(n);
        for (int i = 0; i < n; i++) {
            cin >> pts[i].x >> pts[i].y;    // [-10000, 10000]
        }
        // polygon in ccw order
        vector<point> poly = CH_Andrew(pts);
        int size = (int) ((poly.size() == 1) ? 1 : (poly.size() - 1));
        cout << size << endl;
        for (int i = 0; i < (int) size; i++) {
            // avoid last point (repeated point)
            point p = poly[i];
            cout << p.x << " " << p.y << endl;
        }
    }
