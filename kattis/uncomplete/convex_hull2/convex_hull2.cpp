...
// in this case we have set it to accept collinear points
...
// modified to accept collinear points since in this exercise, we must keep the points that are given to us
// idea -> if the sizes are different, then while(1) or exception
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
      ...
      cin >> x >> y >> c;
      if (c == 'Y') {
          pts[size++] = point(x, y);   // [-1e9, 1e9]
      }
  }
  pts.resize(size);
  vector<point> poly = CH_Andrew(pts);    // already sorts by increasing x and if draw increasing y
