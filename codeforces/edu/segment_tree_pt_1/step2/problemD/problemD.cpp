...
public:
     ...
     */
    int first_above(int x, int i, int j, int current, int l, int r) {
        if (outside(l, r, i, j) || st.at(current) < x) return -1;  // no element >= x
        if (l == r) return l;
        ...
        int rl = first_above(x, i, j, leftC, l, m);
        if (rl != -1) return rl;
        return first_above(x, i, j, rightC, m + 1, r);
    }
...
public:
    ...
    int first_above(int x, int i) {
        return first_above(x, i, n - 1, 1, 0, n - 1);
    }
};
