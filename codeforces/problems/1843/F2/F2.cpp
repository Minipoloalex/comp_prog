...
class Tree {
    ...
    public:
        ...
        Tree(const vector<int>& parents, const vector<int> &vals)
            ...
            pow2aux = vector<vector<T>>(n, vector<T>(log2dist + 1));
            for (int i = 0; i < n; i++) {
                pow2ends[i][0] = par[i];
                for (int j = 0; j < 3; j++) {
                    pow2aux[i][0][j] = vals[i] > 0 ? vals[i] : 0;
                }
                pow2aux[i][0][3] = vals[i];

                for (int j = 4; j < 3+4; j++) {
                    pow2aux[i][0][j] = vals[i] < 0 ? vals[i] : 0;
                }
                pow2aux[i][0][7] = vals[i];
            }
            for (int p = 1; p <= log2dist; p++) {
                for (int i = 0; i < n; i++) {
                    ...
                    if (halfway == -1) {
                        ...
                        pow2aux[i][p] = f(pow2aux[i][p-1], pow2aux[halfway][p-1]);
                    }
                }
            }
        ...
        pair<int,T> lca(int n1, int n2) {
            ...
            if (n1 == n2) {
                return {n1, f(resl, pow2aux[n1][0])};
            }
            for (int i = log2dist; i >= 0; i--) {
                if (pow2ends[n1][i] != pow2ends[n2][i]) {
                    resl = f(resl, pow2aux[n1][i]);
                    resr = f(resr, pow2aux[n2][i]);
                    n1 = pow2ends[n1][i];
                    n2 = pow2ends[n2][i];
                }
            }
            ...
            resl = f(resl, pow2aux[n1][0]);
            resr = f(resr, pow2aux[n2][0]);
            resl = f(resl, pow2aux[ans][0]);
            return {ans, g(resl, resr)};
        }
};
