...
void solve() {
    string s;
    cin >> s;
    prepare_hash_pow(MAX);
    s_hash hsh(s);
    int n = int(s.size());
    auto possible = [&](int med) {
        unordered_map<hash_t, int> mp;
        for (int i = 0; i + med - 1 < n; i++) {
            hash_t hash_value = hsh.combined_hash(i, i + med - 1);
            if (mp.contains(hash_value)) {
                mp[hash_value] = -1;
            }
            else {
                mp[hash_value] = i;
