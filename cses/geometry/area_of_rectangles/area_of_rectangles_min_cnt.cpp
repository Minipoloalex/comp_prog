...
enum UpdateType { ADD, NONE };

// Keeps track of Lazy Updates
struct Tag {
    // Default values
    UpdateType type = NONE;
    int value = 0;   // only important if type is not None
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        int x1, y1, x2, y2;
        cin >> x1 >> y1 >> x2 >> y2;
        evs.push_back({x1, 0, y1, y2});
        evs.push_back({x2, 1, y1, y2});
    }
    vector<Info> base(2*MAX+1);
    LazySegTree<Info,Tag> st(base);
    auto query = [&st]() {
        return st.range_query(L, R).cnt;
    };
    sort(evs.begin(), evs.end());
    int prevX = MIN;
    int64_t ans = 0;
    for (int i = 0; i < int(evs.size()); i++) {
        auto &[x, type, y1, y2] = evs[i];
        int cntYs = query();
        // cout << int64_t(2*MAX+1-cntYs) << " ";
        ans += int64_t(2*MAX+1-cntYs) * (x - prevX);
        if (type == 0) {
            st.range_update(y1 + MAX, y2 + MAX - 1, Tag{ADD, +1});
        }
        else {
            st.range_update(y1 + MAX, y2 + MAX - 1, Tag{ADD, -1});
        }
