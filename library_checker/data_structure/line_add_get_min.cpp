...
class LiChaoTree
{
	private:
		Node * root;
		int min_x, max_x; //the range of the queries (i.e. "domain" of the functions [min_x,max_x])
		
		Node * add_aux(int l, int r, Node * cur, linear_function f){
			if(l>r)return NULL;
		...
		long long get_aux(int l, int r, Node * cur, int x){
			/*
	...
	public:
		...
		*/
		LiChaoTree(int _min_x,int _max_x){
			root = NULL;
			this->min_x = _min_x;
			this->max_x = _max_x;
		}
...
void solve() {
    int n, q;
    cin >> n >> q;
    LiChaoTree t(-MAX, MAX);
    while (n--) {
        int64_t a, b;
        cin >> a >> b;
        t.add(linear_function(a,b));
    }
    while (q--) {
        int tp;
        cin >> tp;
