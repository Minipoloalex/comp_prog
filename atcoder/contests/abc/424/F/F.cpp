...
struct Tag {
    ...
    ll mnB = INT_MAX, mxA = INT_MIN;
	// only important if type is not None

	/* Combining lazy updates in a node */
	void apply(const Tag &update) {
        if (update.type == SET) {
            type = SET;
            mnB = min(mnB, update.mnB);
            mxA = max(mxA, update.mxA);
        }
	}
};
...
Info operator+(const Info &a, const Info &b) {
	return {min(a.mnB, b.mnB), max(a.mxA, b.mxA)};
}
...
void solve() {
    ...
    while (q--) {
        ...
        cin >> a >> b;
        a--;b--;
        int mnB = st.range_query(a, a).mnB;
        int mxA = st.range_query(b, b).mxA;
        bool valid = true;
        if (b > mnB) {  // b <= mnB
            valid = false;
        }
        ...
        if (valid) {
            // type, mnB, mxA
            if (a + 1 <= b - 1) {
                st.range_update(a+1, b-1, Tag{SET, b, a});
            }
        }
        cout << (valid ? "Yes" : "No") << '\n';
    }
